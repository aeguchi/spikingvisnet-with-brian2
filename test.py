...
numInputCells = 10;
#input
layerG=br.PoissonGroup(
         numInputCells,  # N neurons
         Rmax)  # rates
...
sttMonLayerEx = br.StateMonitor(layerEx,'v', record=True);
...
for i in range(nEp):
    ...
    plt.subplot(4,nEp,nEp*1+i+1);
    plt.plot(sttMonLayerEx.t/ms,sttMonLayerEx.v[0]);
    plt.xlim([simTime*i,simTime*(i+1)])
    plt.ylim([Vr*1.01,Vt*0.99])
    #plt.title(str(spkMonLayerEx.num_spikes*1.0/simTime*1000))
    
    plt.subplot(4,nEp,nEp*2+i+1);
    plt.hold(True);
